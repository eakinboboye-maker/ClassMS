...
async function login(email, password) {
  const tokenData = await api("/api/auth/login", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({email,password})});
  const me = await api("/api/auth/me", {headers:{Authorization:`Bearer ${tokenData.access_token}`}});
...
async function markAttendance(lessonSlug) { return api(`/api/jupyterlite/attendance/${lessonSlug}/mark`, {method:"POST", headers: authHeaders()}); }
