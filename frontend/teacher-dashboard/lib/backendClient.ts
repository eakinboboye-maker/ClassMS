...
export class BackendClient {
  ...
  async getGradebook(params?: {
    ...
    if (params?.section_id != null) query.set("section_id", String(params.section_id));
    if (params?.assessment_id != null) query.set("assessment_id", String(params.assessment_id));
    if (params?.published_only != null) query.set("published_only", String(params.published_only));

    const url = `${this.baseUrl}/api/grading/gradebook${query.toString() ? `?${query}` : ""}`;
    const res = await fetch(url, { method: "GET", headers: this.headers() });
    if (!res.ok) throw new Error(`Gradebook fetch failed: ${res.status}`);
    return res.json();
  }

  async getIncidentDashboard(params?: {
    assessment_id?: number;
    user_id?: number;
    ...
    const url = `${this.baseUrl}/api/admin/incidents/dashboard${query.toString() ? `?${query}` : ""}`;
    const res = await fetch(url, { method: "GET", headers: this.headers() });
    if (!res.ok) throw new Error(`Incident dashboard fetch failed: ${res.status}`);
    return res.json();
  }
}
